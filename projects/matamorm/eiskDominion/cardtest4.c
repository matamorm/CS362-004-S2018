...
int main() {
	...
	//returnVal = great_hallCardEffect(currentPlayer, &state, 0);
	returnVal = cardEffect(great_hall, 0, 0, 0, &state, 0, NULL);
	//
	...
	//grab player action count, should be two
	postActionCount =  state.numActions;	
	printf("Current player action count: %d.\n", postActionCount);
	tsAssert(postActionCount == 2);

	//compare originally counted victory piles to current piles
	printf("Comparing victory piles' card counts after great hall.\n");
	for (i = 0; i < 3; i++) {
	...
	printf("Comparing kingdom piles' card counts after great hall.\n");	
	for (i = 0; i < 10; i++) {
		postKCards[i] = supplyCount(kingdomCards[i],&state);
		tsAssert(preKCards[i] == postKCards[i]); 
	}

	printf("\n------- GREAT HALL TESTING END -------\n\n");
